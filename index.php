<?php  
	...
	if(isset($_GET['search']) AND !empty($_GET['search'])) {
	   ...
	   $articles = $bdd->query('SELECT titre, texte, id FROM articles WHERE titre LIKE "%'.$search.'%" OR texte LIKE  "%'.$search.'%" ' );
	   if($articles->rowCount() == 0) {
	      $articles = $bdd->query('SELECT titre FROM articles WHERE CONCAT(titre, texte) LIKE "%'.$search.'%" ORDER BY id DESC');
	   }
	}
